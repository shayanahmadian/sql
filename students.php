...
<?php
	...
	if(isset($_GET['action']) && $_GET['action'] == "edit"){
		$id = $_GET['id'];
		$student_to_update = get_student($id);
	}

	if( isset($_POST['action']) && $_POST['action'] == "delete"){
		delete_student($_POST['id']);
	}
            ...
            <table class="table table-striped table-hover rtl">
                ...
                <tbody class="text-center">
					<?php foreach ($students as $key => $value) { 
						$url = "./students.php?action=edit&id=" . $value['id'];
						?>
                        <tr>
                            <td><?= $value['id'] ?></td>
...
<form method="post" action="./students.php" style="display: none;">
      ...
      <div class="modal-body rtl text-right">
        <form method="post" action="./students.php">
			...
			<div class="form-group">
				<label>نام :</label>
				<input type="text" name="firstname" class="form-control"
					value="<?=$student_to_update['firstname']?>" 
					placeholder="نام را وارد کنید" required>
			</div>
			...
			<div class="form-group">
				<label>نام خانوادگی :</label>
				<input type="text" placeholder="نام خانوادگی را وارد کنید" 
				value="<?=$student_to_update['lastname']?>"
				name="lastname" class="form-control" required>
			</div>
			...
			<div class="form-group">
				<label>شماره دانشجویی :</label>
				<input type="text"
				value="<?=$student_to_update['student_number']?>" 
				placeholder="سن را وارد کنید" name="student_number" class="form-control" required>
			</div>
